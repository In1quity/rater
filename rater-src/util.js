...
var filterAndMap = function (array, filterPredicate, mapTransform) {
	return array.reduce((accumulated, currentVal, currentIndex) => {
...
function classMask(classVal) {
	...
	switch (classVal.toLowerCase()) {
		...
		case 'meta':
			return (
				classVal.slice(0, 1).toUpperCase() + classVal.slice(1).toLowerCase()
			);
		case 'image':
		case 'img':
		case 'file':
...
function importanceMask(importance) {
	...
	return (
		importance.slice(0, 1).toUpperCase() + importance.slice(1).toLowerCase()
	);
}
