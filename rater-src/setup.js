			...
			return bannersPromise.then(allBanners => {
				...
				return filterAndMap(
					...
					template => {
						...
						return (
							allBanners.withRatings.includes(mainText) ||
					...
					template => {
						...
						if (allBanners.wrappers.includes(mainText)) {
							template.redirectTarget = mw.Title.newFromText(
								'Template:Subst:' + mainText,
							);
						}
						if (
							allBanners.withoutRatings.includes(mainText) ||
	...
	var subjectPageCheckPromise = API.get({
		...
		.then(response => {
			...
			const redirectTarget =
				(response.query.redirects && response.query.redirects[0].to) || false;
			...
			const hasCategory = category =>
				page.categories &&
				page.categories.find(cat => cat.title === 'Category:' + category);
			return {
				redirectTarget,
				...
				.then(function (result) {
					...
					if (prediction === 'FA' || prediction === 'GA') {
						return {
							...
							probability:
								(
									(probabilities.FA + probabilities.GA + probabilities.B) *
									100
								).toFixed(1) + '%',
						};
					}
	...
	$.when(
		...
		function (
			...
			if (
				oresPredicition &&
				subjectPageCheck &&
				!subjectPageCheck.isGA &&
				!subjectPageCheck.isFA &&
				!subjectPageCheck.isFL
			) {
