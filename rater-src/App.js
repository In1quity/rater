...
(function App() {
	...
	const showMainWindow = data => {
		...
		document
			...
			.classList.add('rater-mainWindow-open');
		// Open the window
		windowManager.openWindow('main', data).closed.then(result => {
			...
			document
				.getElementsByTagName('body')[0]
				.classList.remove('rater-mainWindow-open');
			// Restart if needed
			...
			if (result && result.success) {
				...
				if (result.upgradedStub) {
					$message.append(
						...
						$('<span>').text(
							'Note that the article appears to be tagged as a stub.',
						),
					);
				}
				mw.notify($message, {
	...
	const showSetupError = (code, jqxhr) =>
		OO.ui.alert(makeErrorMsg(code, jqxhr), {
