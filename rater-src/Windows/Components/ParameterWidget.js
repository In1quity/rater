...
function ParameterWidget(parameter, paramData, config) {
	...
	switch (this.allowedValues.length) {
		...
		case 2:
			var isFirstAllowedVal =
				...
				this.allowedValues
					.map(normaliseYesNo)
					.indexOf(normaliseYesNo(parameter.value)) === 0;
			var isSecondAllowedVal =
				this.allowedValues.indexOf(parameter.value || null) === 1 ||
				this.allowedValues
					.map(normaliseYesNo)
					.indexOf(parameter.value ? normaliseYesNo(parameter.value) : null) ===
					1;
	...
	this.editLayout = new OO.ui.FieldLayout(this.editLayoutControls, {
		...
		align: 'top',
		help:
			(this.paramData.description && this.paramData.description.en) || false,
		helpInline: true,
	}).toggle();
	this.editLayout.$element
		.find('label.oo-ui-inline-help')
		.css({ margin: '-10px 0 5px 10px' });
	...
	this.invalidIcon = new OO.ui.IconWidget({
		...
		flags: 'destructive',
		$element: $(
			"<span style='margin: 0 5px 0 -5px; min-width: 16px; width: 16px;'>",
		),
	...
	this.$element = $('<div>')
		.addClass('rater-parameterWidget')
		.css({
			width: 'unset',
...
ParameterWidget.prototype.onCheckboxChange = function (
	isSelected,
	isIndeterminate,
) {
	if (isIndeterminate) {
...
ParameterWidget.prototype.setValue = function (val) {
	...
	if (this.checkbox) {
		...
		var isSecondAllowedVal =
			...
			this.allowedValues
				.map(normaliseYesNo)
				.indexOf(val ? normaliseYesNo(val) : null) === 1;
		var isIndeterminate = !isFirstAllowedVal && !isSecondAllowedVal;
