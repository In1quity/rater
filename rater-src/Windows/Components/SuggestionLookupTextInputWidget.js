...
var SuggestionLookupTextInputWidget = function SuggestionLookupTextInputWidget(
	...
	this.suggestions = Array.isArray(config.suggestions)
		? config.suggestions
		: [];
...
SuggestionLookupTextInputWidget.prototype.setSuggestions = function (
	suggestions,
) {
	if (!Array.isArray(suggestions)) {
		if (suggestions != null) {
			console.warn(
				'[Rater] SuggestionLookupTextInputWidget.prototype.setSuggestions called with a non-array value:',
				suggestions,
			);
		}
		return;
...
SuggestionLookupTextInputWidget.prototype.getLookupRequest = function () {
	var deferred = $.Deferred().resolve(
		new RegExp('\\b' + mw.util.escapeRegExp(this.getValue()), 'i'),
	);
	return deferred.promise({ abort: function () {} });
};

// ???
SuggestionLookupTextInputWidget.prototype.getLookupCacheDataFromResponse =
	function (response) {
		return response || [];
	};

// Is passed data from #getLookupRequest, returns an array of menu item widgets
SuggestionLookupTextInputWidget.prototype.getLookupMenuOptionsFromData =
	function (pattern) {
		var labelMatchesInputVal = function (suggestionItem) {
			return (
				pattern.test(suggestionItem.label) ||
		...
		return this.suggestions
			.filter(labelMatchesInputVal)
			.map(makeMenuOptionWidget);
	};
