...
function TopBarWidget(config) {
	...
	this.searchBox = new SuggestionLookupTextInputWidget({
		...
		$element: $(
			"<div style='display:inline-block; margin:0 -1px; width:calc(100% - 55px);'>",
		),
	...
	this.addBannerButton = new OO.ui.ButtonWidget({
		...
		flags: 'progressive',
		$element: $(
			"<span style='float:right;margin: 0;transform: translateX(-12px);'>",
		),
	});
	var $searchContainer = $(
		"<div style='display:inline-block; flex-shrink:1; flex-grow:100; min-width:250px; width:50%;'>",
	...
	this.setAllDropDown = new OO.ui.DropdownWidget({
			...
			items: [
				...
				new OO.ui.MenuOptionWidget({
					...
					label: new OO.ui.HtmlSnippet(
						'<span style="color:#777">(no class)</span>',
					),
				}),
				...appConfig.bannerDefaults.classes.map(
					classname =>
						new OO.ui.MenuOptionWidget({
				...
				...appConfig.bannerDefaults.importances.map(
					importance =>
						new OO.ui.MenuOptionWidget({
		...
		$element: $(
			'<span style="width:auto;display:inline-block;float:left;margin:0" title=\'Set all...\'>',
		),
		$overlay: this.$overlay,
	});
