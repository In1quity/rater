...
function BannerWidget(template, config) {
	...
	this.redirectTargetMainText =
		template.redirectTarget && template.redirectTarget.getMainText();
	...
	this.hasImportanceRatings =
		template.importances && template.importances.length;
	...
	this.mainLabelPopupButton = new OO.ui.PopupButtonWidget({
		...
		$element: $(
			"<span style='display:inline-block;width:48%;margin-right:0;padding-right:8px'>",
		),
	...
	this.mainLabelPopupButton.$element
		...
		.first()
		.css({ 'font-size': '110%' })
		.find('span.oo-ui-labelElement-label')
	...
	if (this.isShellTemplate) {
				...
				items: [
					new OO.ui.MenuOptionWidget({
						...
						label: new OO.ui.HtmlSnippet(
							`<span style="color:#777">(${config.isArticle ? 'no class' : 'auto-detect'})</span>`,
						),
					}),
					...globalConfig.bannerDefaults.classes.map(
						classname =>
							new OO.ui.MenuOptionWidget({
								data: classname,
								label: classname,
		...
		var shellClassParam = template.parameters.find(
			parameter => parameter.name === 'class',
		);
		this.classDropdown
			.getMenu()
			.selectItemByData(shellClassParam && classMask(shellClassParam.value));
			...
			menu: {
				items: [
					new OO.ui.MenuOptionWidget({
						data: null,
						label: new OO.ui.HtmlSnippet(
							`<span style="color:#777">(${config.isArticle ? 'inherit from shell' : 'auto-detect'})</span>`,
						),
					}),
		...
		var classParam = template.parameters.find(
			parameter => parameter.name === 'class',
		);
		this.classDropdown
			.getMenu()
			.selectItemByData(classParam && classMask(classParam.value));
	}

	if (this.hasImportanceRatings) {
		this.importanceDropdown = new DropdownParameterWidget({
			label: new OO.ui.HtmlSnippet(
				'<span style="color:#777">Importance</span>',
			),
			menu: {
				items: [
					new OO.ui.MenuOptionWidget({
						data: null,
						label: new OO.ui.HtmlSnippet(
							`<span style="color:#777">(${config.isArticle ? 'no importance' : 'auto-detect'})</span>`,
						),
					}),
					...template.importances.map(
						importance =>
							new OO.ui.MenuOptionWidget({
								data: importance,
								label: importance,
		...
		var importanceParam = template.parameters.find(
			parameter => parameter.name === 'importance',
		);
	...
	var parameterWidgets = filterAndMap(
		...
		param =>
			new ParameterWidget(param, template.paramData[param.name], {
				$overlay: this.$overlay,
			}),
	);
...
BannerWidget.prototype.setChanged = function () {
	...
	this.emit('changed');
	if (
		this.mainText === 'WikiProject Biography' ||
		this.redirectTargetMainText === 'WikiProject Biography'
	) {
...
BannerWidget.prototype.getAddParametersInfo = function (
	nameInputVal,
	valueInputVal,
) {
	var name =
		(nameInputVal && nameInputVal.trim()) ||
		this.addParameterNameInput.getValue().trim();
	var paramAlreadyIncluded =
		name === 'class' ||
		name === 'importance' ||
		(name === '1' && this.isShellTemplate) ||
		this.parameterList
			.getParameterItems()
			.some(paramWidget => paramWidget.name === name);
	var value =
		(valueInputVal && valueInputVal.trim()) ||
		this.addParameterValueInput.getValue().trim();
	var autovalue =
		(name && this.paramData[name] && this.paramData[name].autovalue) || null;
	return {
...
BannerWidget.prototype.onAddParameterNameChange = function () {
	...
	var allowedValues =
		...
		this.paramData[name].allowedValues.map(val => {
	...
	this.addParameterLayout.setNotices(
		validName && isAutovalue ? ['Parameter value will be autofilled'] : [],
	);
	// Show error is the banner already has the parameter set
	this.addParameterLayout.setErrors(
		isAlreadyIncluded ? ['Parameter is already present'] : [],
	);
};
...
BannerWidget.prototype.onParameterAdd = function () {
	let { validName, validValue, name, value, autovalue } =
		this.getAddParametersInfo();
...
BannerWidget.prototype.updateAddParameterNameSuggestions = function () {
	...
	this.parameterList
		.getParameterItems()
		.forEach(paramWidget => (paramsInUse[paramWidget.name] = true));
...
BannerWidget.prototype.bypassRedirect = function () {
	...
	// Update title label
	this.mainLabelPopupButton.setLabel(
		`{{${this.redirectTargetMainText}}}${this.inactiveProject ? ' (inactive)' : ''}`,
	);
	// Update properties
...
BannerWidget.prototype.makeWikitext = function () {
	...
	var classItem =
		(this.hasClassRatings || this.isShellTemplate) &&
	...
	return (
		'{{' +
		this.name +
		((this.hasClassRatings || this.isShellTemplate) && classVal != null
			? `${pipe}class${equals}${classVal || ''}`
			: '') +
		(this.hasImportanceRatings && importanceVal != null
			? `${pipe}importance${equals}${importanceVal || ''}`
			: '') +
		this.parameterList
			.getParameterItems()
