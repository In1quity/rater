	...
	this.preferences = {
			...
			input: new OO.ui.ButtonSelectWidget({
				items: [
					new OO.ui.ButtonOptionWidget({
						...
						label: 'Default',
						title:
							'Uses the same setting as if you manually edited the page, as per Special:Preferences',
					}),
					new OO.ui.ButtonOptionWidget({
						data: 'watch',
						label: 'Always',
		...
		resetCache: {
			input: new OO.ui.ButtonWidget({
				label: 'Reset cache',
				title:
					'Remove cached data, including list of WikiProjects and template parameters',
				flags: ['destructive'],
			}),
		},
	};
		...
		switch (input && input.constructor.name) {
			...
			case 'MwWidgetsNamespacesMultiselectWidget':
				...
				value.forEach(ns =>
					input.addTag(
						ns.toString(),
						ns === 0 ? '(Main)' : config.mw.wgFormattedNamespaces[ns],
					),
				);
				break;
		}
...
PrefsFormWidget.prototype.onResetCacheClick = function () {
	OO.ui
		.confirm(
			'After reseting cache, Rater will close and restart. Any changes made will be discarded.',
		)
		.then(confirmed => {
			if (confirmed) {
