...
var BannerListWidget = function BannerListWidget(config) {
	...
	this.$element
		.addClass('rater-bannerListWidget')
		.css({ padding: '20px 10px 16px 10px' });
...
BannerListWidget.prototype.syncShellTemplateWithBiographyBanner = function (
	biographyBanner,
) {
	biographyBanner =
		biographyBanner ||
		this.items.find(
			banner =>
				banner.mainText === 'WikiProject Biography' ||
	...
	const bannerShellTemplate = this.items.find(
		banner =>
			banner.mainText === config.shellTemplates[0] ||
	...
	paramsToSync.forEach(paramToSync => {
		const [biographyParam, shellParam] = [
			biographyBanner,
			bannerShellTemplate,
		].map(banner =>
		...
		const paramSyncValue = paramToSync.normalise
			? normaliseYesNo(biographyParam.value)
			: biographyParam.value;
			...
			bannerShellTemplate.parameterList.addItems(
				[
					new ParameterWidget(
						...
						bannerShellTemplate.paramData &&
							bannerShellTemplate.paramData[paramToSync.name],
					),
				],
				index,
			);
...
BannerListWidget.prototype.addShellTemplateIfNeeeded = function () {
	if (!this.items.some(banner => banner.isShellTemplate)) {
		BannerWidget.newFromTemplateName(
			...
			OO.ui.mixin.GroupElement.prototype.addItems.call(
				this,
				[shellBannerWidget],
				0,
			);
...
BannerListWidget.prototype.autofillClassRatings = function (config) {
	...
	if (
		!this.preferences.autofillClassFromOthers &&
		!this.preferences.autofillClassFromOres &&
		!config.forBannerShell
	) {
		return;
	}
	// Check what banners already have
	const uniqueClassRatings = uniqueArray(
		filterAndMap(
			this.items,
			banner => {
	...
	this.items.forEach(banner => {
		...
		if (
			config.forBannerShell &&
			!banner.isShellTemplate &&
			classItem.getData() === autoClass
		) {
...
BannerListWidget.prototype.autofillImportanceRatings = function () {
	...
	const isRegularArticle =
		this.pageInfo &&
		this.pageInfo.isArticle &&
		!this.pageInfo.redirect &&
		!this.pageInfo.isDisambig;
	if (!isRegularArticle) {
		return;
	}
	...
	this.items.forEach(banner => {
		...
		const importanceItem = banner.importanceDropdown
			.getMenu()
			.findSelectedItem();
...
BannerListWidget.prototype.makeWikitext = function () {
	...
	var shellParam1 = new ParameterWidget({
		...
		value:
			...
			bannersWikitext +
			'\n' +
			(shellTemplate.nonStandardTemplates
				? shellTemplate.nonStandardTemplates + '\n'
				: ''),
	});
